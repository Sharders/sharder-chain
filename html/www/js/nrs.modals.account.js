...
var NRS = (function(NRS, $) {
	...
	NRS.processAccountModalData = function(account) {
		if (account.unconfirmedBalanceNQT == "0") {
			...
			var isGenesisRP =  ("SSA-6QF9-2HA9-3ZCB-83XGP" == $("#user_info_modal_account").html());
			$("#user_info_modal_account_balance").html(isGenesisRP? "未知" : (NRS.formatAmount(account.unconfirmedBalanceNQT) + " SS"));
		}
        ...
        NRS.sendRequest("getBlockchainTransactions", {
			...
			if (response.transactions && response.transactions.length) {
				...
				for (var i = 0; i < response.transactions.length; i++) {
                    ...
                    var receiving;
					if (/^SSA\-/i.test(String(NRS.userInfoModal.user))) {
					...
					if(NRS.isCoinBaseTransaction(transaction)){
                        receiving = true;
					}
					rows += "<tr>" +
		...
		NRS.sendRequest("getDGSGoods", {
			...
			if (response.goods && response.goods.length) {
				for (var i = 0; i < response.goods.length; i++) {
					...
					rows += "<tr><td><a href='#' data-goto-goods='" + NRS.escapeRespStr(good.goods) + "' data-seller='" + NRS.escapeRespStr(NRS.userInfoModal.user) + "'>" + NRS.escapeRespStr(good.name) + "</a></td><td class='numeric'>" + NRS.formatAmount(good.priceNQT, false, false, priceDecimals) + " SS</td><td class='numeric'>" + NRS.format(good.quantity, false, quantityDecimals) + "</td></tr>";
				}
			}
