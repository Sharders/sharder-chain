...
var NRS = (function(NRS, $, undefined) {
	...
	function initSlideTouch(){
		...
		var slider = { //判断设备是否支持touch事件
			touch:('ontouchstart' in window) || window.DocumentTouch && document instanceof DocumentTouch, slider:document.getElementById('sidebar_menu'), //事件
			events:{
				index:0, //显示元素的索引
				...
				start:function(event){
					...
					this.slider.addEventListener('touchend',this,false); }, //移动
				move:function(event){ //当屏幕有多个touch或者页面被缩放过，就不执行move操作
					...
					if(isScrolling === 0){
						...
						this.slider.className = 'cnt'; this.slider.style.left = -this.index*600 + endPos.x + 'px';
					}
				},
				//滑动释放
				end:function(event){
		...
		loadConstantsPromise.then(function() {
			var getStatePromise = new Promise(function(resolve) {
				...
				NRS.sendRequest("getState", {
					...
					if (isTestnet != "Devnet" && isTestnet != "Testnet") {
						NRS.network = "Mainnet";
						$(".testnet_only").hide();
					} else {
                        var testnetWarningDiv;
                        var network;
                        var message;
                        if (isTestnet == "Testnet") {
                            testnetWarningDiv = $("#testnet_warning");
                            network="#testnet_warning";
                            message="testnet";
                            NRS.network="Testnet";
                        }
                        if (isTestnet == "Devnet") {
                            testnetWarningDiv = $("#devnet_warning");
                            network="#devnet_warning";
                            message="devnet";
                            NRS.network="Devnet";
                        }
						NRS.isTestNet = true;

						var warningText = testnetWarningDiv.text() + " The "+message+" peer port is " + peerPort + (isOffline ? ", the peer is working offline." : ".");
						testnetWarningDiv.text(warningText);
						$(".testnet_only, #testnet_login,"+network).show();
					}
	...
	function mobileHide(){
		if (window["cordova"] && "iOS" == device.platform) {
	...
	NRS.handleBlockchainStatus = function(response, callback) {
		...
		if (firstTime) {
			var versionTxt = (window["cordova"] && "iOS" == device.platform) ? NRS.state.version : NRS.state["fullVersion"];
	...
	$("#logo, .navbar-main , .hrefbtn").on("click", "li,a", function(e, data) {
		...
		console.log("NRS.page",NRS.page[page]);
		if (NRS.pages[page]) {
			NRS.pageLoading();
