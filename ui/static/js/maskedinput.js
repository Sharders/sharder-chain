 ...
 if (tests[i]) {
                ...
                function androidInputEvent () {
                    ...
                    if (oldVal && oldVal.length && oldVal.length > curVal.length) {
                        ...
                        if (pos.end === 0) for (;pos.end < firstNonMaskPos && !tests[pos.end];) pos.end++;
                        input.caret(pos.end, pos.end);
                    } else {
                        var curValUpper = curVal.toUpperCase();
                        var addressStart = curValUpper.indexOf("SSA-", 4);
                        if (addressStart > 0) {
                            var insertedAddress = curValUpper.substr(addressStart, 24);
                            if (/SSA\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}/.test(insertedAddress)) {
                ...
                function keydownEvent (e) {
                    ...
                    if (e.keyCode == 8) {
                        ...
                        if (settings.unmask !== false) {
                            // backspace, remove
                            if ((pos.begin == 0 && pos.end == 24) || (currentInput == "SSA-____-____-____-_____" && pos.begin == 4)) {
                                input.val("");
                                $(this).trigger("unmask");
                                return;
                            }
                    ...
                    if (!input.prop("readonly")) {
                        ...
                        k === 8 || k === 46 || iPhone && k === 127 ? (pos = input.caret(),
...
if (tests[i]) {
 ...
 if (c = test.charAt(pos - 1).toUpperCase(),
                        tests[i].test(c)) {
                ...
                input.bind("keyup.remask", function (e) {
                    if (input.val().toLowerCase() == "nxt-") {
                        input.val("").mask("SSA-****-****-****-*****")./* unbind(".remask"). */trigger("focus");
                    }
                }).bind("paste.remask", function (e) {
                    setTimeout(function () {
                        var newInput = input.val();
                        var pastedData = newInput.substring(4).toUpperCase();
                        if (/CC\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}/i.test(pastedData)) {
                            var newAddress = String(pastedData.match("CC\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}"));
                            input.val(newAddress);
                            checkVal(true);
                        } else if (/^CC\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}/i.test(newInput) || /^CC[A-Z0-9]{17}/i.test(newInput)) {
                            input.mask("SSA-****-****-****-*****").trigger("checkRecipient")/* .unbind(".remask") */;
                        }
                    }, 0);
                });
