        ...
        if ($('#' + id).length == 0) {
            ...
            $.each(sidebarMenu.find('> li'), function(key, elem) {
                ...
                if (!inserted && compPos && desiredPosition <= parseInt(compPos)) {
                    if(isloadToSidebar(id)) $(menuHTML).insertBefore(elem);
                    inserted = true;
                }
            });
            if (!inserted && isloadToSidebar(id)) {
