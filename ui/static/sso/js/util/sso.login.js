...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.login = function (isPassphraseLogin, id, vue, callback, isAccountSwitch, isSavedPassphrase) {
        ...
        // NRS.logConsole("login calling getBlockchainStatus");
        NRS.sendRequest("getBlockchainStatus", {}, function (response) {
            ...
            // NRS.logConsole("calling " + accountRequest);
            NRS.sendRequest(accountRequest, requestVariable, function (response, data) {
                // NRS.logConsole(accountRequest + " response received");
                if (!response.errorCode) {
                ...
                //     if (isPassphraseLogin == 1 && response["sharder.HubBindAddress"] == NRS.accountRS) {
                //     } else {
                //         $("#config").remove();
                //     }
                //     /*if (isPassphraseLogin == 1 && response.accountRS == NRS.accountRS) {
                //     }else{
                //         $("#config").remove();
                //     }*/
                // });
                ...
                if (!NRS.account) {
                    ...
                    // });
                    // NRS.spinner.stop();
                    vue.$message.error("无法找到您的帐户地址" + data && data.account ? String(data.account) : "");
                    return;
    ...
    NRS.initPluginWarning = function () {
        if (NRS.activePlugins) {
            ...
            html += "Plugins are not sandboxed or restricted in any way and have full accesss to your client system including your CDWH passphrase.";
            html += "</p>";
            ...
            html += "you can loose your CDWH! In doubt don't run plugins with accounts ";
            html += "used to store larger amounts of CDWH now or in the future.";
            html += "</p>";
            html += "</div>";
