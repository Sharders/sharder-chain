...
var NRS = (function(NRS, $, undefined) {
				...
				if (responsePoll.transaction) {
					NRS.sendRequest("getPhasingPollVote", {
						...
						if (vm == 1) {
							$popoverTypeTR.find("td:first").html($.t('sso.accounts', 'Accounts') + ":");
							$popoverTypeTR.find("td:last").html(String(attachment.phasingWhitelist ? attachment.phasingWhitelist.length : ""));
							votesFormatted = NRS.convertToNXT(responsePoll.result) + " / " + NRS.convertToNXT(attachment.phasingQuorum) + " CDWH";
							$popoverVotesTR.find("td:last").html(votesFormatted);
						}
						if (mbModel == 1) {
							if (minBalance > 0) {
								minBalanceFormatted = NRS.convertToNXT(minBalance) + " CDWH";
    ...
    NRS.getTransactionRowHTML = function(t, actions, decimals) {
        ...
        if(NRS.isCoinBaseTransaction(t)){
            sign = 1;
    ...
    NRS.isCoinBaseTransaction = function(t){
    	if(typeof t !== 'object'){
    		return false;
		}
		if(t.type === 9 && t.subtype === 0){
			return true;
		}
	};
	...
	function showTxType(typeDict){

		return "payment" ==  typeDict.i18nKeyTitle //支付
				|| "messaging_voting_aliases" ==  typeDict.i18nKeyTitle //信息/投票/别名
				//|| "marketplace" == typeDict.i18nKeyTitle //市场
				//|| "account_control" == typeDict.i18nKeyTitle //帐户控制
				//|| "monetary_system" == typeDict.i18nKeyTitle //货币系统
				//|| "tagged_data" == typeDict.i18nKeyTitle //数据云
				//|| "shuffling" == typeDict.i18nKeyTitle //混币
				// || "asset_exchange" == typeDict.i18nKeyTitle; //资产管理
	}
	...
	NRS.buildTransactionsTypeNavi = function() {
		...
		$.each(NRS.transactionTypes, function(typeIndex, typeDict) {
			if(!showTxType(typeDict)) return; //是否需要显示
