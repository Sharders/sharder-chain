        ...
        private void downloadPeer() throws InterruptedException {
            try {
                ...
                if (connectedPublicPeers.size() <= numberOfForkConfirmations) {
                    Logger.logMessage("No enough peers[limit size=" + (numberOfForkConfirmations + 1) + ",current connected size=" + connectedPublicPeers.size() + "], break syn blocks...");
                    return;
                }
                peerHasMore = true;
    ...
    private boolean addConchGenesisBlock() {
        ...
        try {
            ...
            for (int i = 0; i < ConchGenesis.GENESIS_RECIPIENTS.length; i++) {
                TransactionImpl transaction = new TransactionImpl.BuilderImpl((byte) 0, ConchGenesis.CREATOR_PUBLIC_KEY,
                        ConchGenesis.GENESIS_AMOUNTS[i] * Constants.ONE_SS, 0, (short) 0,
                        Attachment.ORDINARY_PAYMENT)
                        .timestamp(0)
            ...
            byte[] state = ByteUtil.hexStringToBytes("2accbd9ca576a9337fa07a134daf0803056f814dc70736f6b9e1d055e7bd13a2");
...
//                    ConchGenesis.CREATOR_PUBLIC_KEY, new byte[64], null, transactions,ConchGenesis.CREATOR_SP);
    ...
    private void pushBlock(final BlockImpl block) throws BlockNotAcceptedException {
        ...
        try {
            ...
            try {
                ...
                addBlock(block);
                if (!FastByteComparisons.equal(previousLastBlock.getStateRoot(), SourceAdapter.getStateRoot()))
                    throw new BlockchainProcessor.BlockNotAcceptedException("state root doesn't roll back after getting new block state root", block);
    ...
    private final static long AMOUNT_LIMIT = 10000L;
    private boolean validForgeAccountTranscation(TransactionImpl transaction){
       if(Account.getId(transaction.getSenderPublicKey()) != RewardIssuer.FORGE_ACCOUNT_ID) return true;

       if(!ConchGenesis.isFoundAccount(transaction.getRecipientId())
        && transaction.getAmountNQT() >= AMOUNT_LIMIT * Constants.ONE_SS)
           return false;
    ...
    private byte[] accept(BlockImpl block, List<TransactionImpl> validPhasedTransactions, List<TransactionImpl> invalidPhasedTransactions,
        ...
        try {
            ...
            return SourceAdapter.getStateRoot();
        } finally {
    ...
    List<BlockImpl> popOffTo(Block commonBlock) {
        ...
        try {
            ...
            try {
                ...
                while (block.getId() != commonBlock.getId() && block.getId() != ConchGenesis.GENESIS_BLOCK_ID) {
    ...
    void generateBlock(String secretPhrase, int blockTimestamp) throws BlockNotAcceptedException {
        ...
        byte[] state = getBlockStateRoot(block);
        ...
        try {
            ...
            Logger.logDebugMessage("Account " + Long.toUnsignedString(block.getGeneratorId()) + " generated block " + block.getStringId()
                    + " at height " + block.getHeight() + " timestamp " + block.getTimestamp() + " fee " + ((float)block.getTotalFeeNQT())/Constants.ONE_SS);
    ...
    private byte[] getBlockStateRoot(BlockImpl block) throws BlockNotAcceptedException {
        try {

            Db.db.beginTransaction();
            BlockImpl previousLastBlock = blockchain.getLastBlock();
            byte[] stateRoot = previousLastBlock.getStateRoot();
            if (!FastByteComparisons.equal(SourceAdapter.getStateRoot(), stateRoot))
                throw new BlockchainProcessor.BlockNotAcceptedException("current state doesn't match with precious block state root ", previousLastBlock);

            //block apply
            Account generatorAccount = Account.addOrGetAccount(block.getGeneratorId());
            generatorAccount.apply(block.getGeneratorPublicKey());
            generatorAccount.addToBalanceAndUnconfirmedBalanceNQT(AccountLedger.LedgerEvent.BLOCK_GENERATED, block.getPreviousBlockId(), block.getTotalFeeNQT());
            generatorAccount.addToForgedBalanceNQT(block.getTotalFeeNQT());
            ...
            for (TransactionImpl transaction : block.getTransactions()) {
                try {
                    if (transaction.getType().getType() == TransactionType.TYPE_CONTRACT) {
                        RepositoryImpl repository = new RepositoryImpl(SourceAdapter.getStateRoot());
                        TransactionExecutor executor = new TransactionExecutor
                                (transaction, block.getGeneratorPublicKey(), repository, new BlockStoreDummy(),
                                        new ProgramInvokeFactoryImpl(), block);
            ...
            stateRoot = SourceAdapter.getStateRoot();
            Db.db.rollbackTransaction();
            return stateRoot;
        } catch (TransactionNotAcceptedException e) {
                                ...
                                if (validate && currentBlockId != ConchGenesis.GENESIS_BLOCK_ID) {
                                    ...
                                    for (TransactionImpl transaction : currentBlock.getTransactions()) {
                                        ...
                                        if (currentBlock.getHeight() > Constants.NQT_BLOCK
                                            ...
                                            throw new ConchException.NotValidException("Transaction bytes cannot be parsed back to the same transaction: "
                                        ...
                                        if (!Arrays.equals(transactionBytes, TransactionImpl.newTransactionBuilder(transactionJSON).build().bytes())) {
                                            throw new ConchException.NotValidException("Transaction JSON cannot be parsed back to the same transaction: "
                                                    + transaction.getJSONObject().toJSONString());
                                        }
                                    }
                                }
