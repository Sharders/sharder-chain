...
final class BlockImpl implements Block {
    ...
    private void calculateBaseTarget(BlockImpl previousBlock) {
        ...
        if (previousBlock.getHeight() <= Constants.SHUFFLING_BLOCK) {
            ...
            if (blocktimeAverage > 480) {
                baseTarget = (prevBaseTarget * Math.min(blocktimeAverage, Constants.MAX_BLOCKTIME_LIMIT)) / 480;
            } else {
                baseTarget = prevBaseTarget - prevBaseTarget * Constants.BASE_TARGET_GAMMA
                        * (480 - Math.max(blocktimeAverage, Constants.MIN_BLOCKTIME_LIMIT)) / 48000;
            }
