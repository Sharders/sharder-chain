...
public class Rule implements Serializable {
    ...
    private static Map<String,Object> toMap(NodeList nodeList){
        ...
        for(int i=0;i<nodeList.getLength();i++){
            ...
            if(nodeList.item(i).getNodeType() == 1
                ...
                result.put(nodeList.item(i).getNodeName(), stringToNumber(nodeList.item(i).getChildNodes().item(0).getNodeValue()));
                continue;
            }
    ...
    private static boolean validate(Map<String,Object> map,Object value){
        if(value instanceof Long){
            ...
            if((long)value < max && (long)value > min){
                ...
                Logger.logDebugMessage("validate forge pool rule failed,min " + min + "max " + max + "value" + value);
                return false;
            }
    ...
    public static Map<Long,Long> getRewardMap(Long creator, Long poolId,Long amount, Map<Long,Long> map){
        ...
        if(((Map<String,Object>)ruleMap.get("reward")).get("max") instanceof Float){
            creatorAmount = Math.round(amount * (1 - (Float)((Map<String,Object>)ruleMap.get("reward")).get("max")));
        }else {
            creatorAmount = Math.round(amount * (1 - (Double)((Map<String,Object>)ruleMap.get("reward")).get("max")));
        }
