...
import org.conch.vm.program.listener.CompositeProgramListener;
import org.conch.vm.program.listener.ProgramListenerAware;
import org.conch.vm.program.listener.ProgramStorageChangeListener;
import org.conch.vm.trace.ProgramTrace;
import org.conch.vm.trace.ProgramTraceListener;
import org.conch.vm.util.ByteArraySet;
import org.conch.vm.util.ByteUtil;
import org.conch.vm.util.FastByteComparisons;
import org.conch.vm.util.Utils;
...
public class Program {
    ...
    public Program(byte[] codeHash, byte[] ops, ProgramInvoke programInvoke, Transaction transaction) {
        ...
        traceListener = new ProgramTraceListener(false);
        this.memory = setupProgramListener(new Memory());
        this.stack = setupProgramListener(new Stack());
        this.storage = setupProgramListener(new Storage(programInvoke));
        this.trace = new ProgramTrace(false, programInvoke);
    }

    public ProgramPrecompile getProgramPrecompile() {
        // TODO wj it should like the follow code
    ...
    private InternalTransaction addInternalTx(byte[] nonce, DataWord gasLimit, byte[] senderAddress, byte[] receiveAddress,
        ...
        if (transaction != null) {
            ...
            result = getResult().addInternalTransaction(transaction.getFullHash().getBytes(), getCallDeep(), senderNonce,
    ...
    public void createContract(DataWord value, DataWord memStart, DataWord memSize) {
        ...
        long storageCost = getLength(code) * new GasCost().getCREATE_DATA();
        long afterSpend = programInvoke.getGas().longValue() - storageCost - result.getGasUsed();
        if (getLength(code) > Integer.MAX_VALUE) {
    ...
    public void callToAddress(MessageCall msg) {
        ...
        if (isNotEmpty(programCode)) {
            ...
            VM vm = new VM();
            Program program = new Program(getStorage().getCodeHash(codeAddress), programCode, programInvoke, internalTx);
            vm.play(program);
            result = program.getResult();
