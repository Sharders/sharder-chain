...
public class CreateApiHtmlServlet extends APIServlet.APIRequestHandler {

    static final CreateApiHtmlServlet instance = new CreateApiHtmlServlet();

    private CreateApiHtmlServlet() {
        super(new APITag[] {}, "rootPath");
    }
    ...
    private static final String header1 =
            ...
            "    <link href='./static/highlight.style.css' rel='stylesheet' type='text/css' />\n" +
            "    <style type='text/css'>\n" +
            ...
            "               <li><a href='https://sharder.org/wiki/CONCH_API' target='_blank' style='margin-left:20px;'>Wiki Docs</a></li>\n" +
            "           </ul>\n" +
            "       </div>\n" +
            "   </div>\n" +
            "</div>\n" +
    ...
    private static final String footer1 =
            ...
            "<script src='./static/highlight.pack.js' type='text/javascript'></script>\n" +
            "<script src='./static/ats.js' type='text/javascript'></script>\n" +
            "<script src='./static/ats.util.js' type='text/javascript'></script>\n" +
            "<script>\n" +
    ...
    private static List<String> allRequestTypes = null;

    private static SortedMap<String, SortedSet<String>> requestTags = null;
    ...
    private static String form(HttpServletRequest req, String requestType, boolean singleView, APIServlet.APIRequestHandler requestHandler) {
        ...
        buf.append("<a style='font-weight:normal;font-size:14px;color:#777;' href='https://sharder.org/wiki/CONCH_API#");
        ...
        String path = (req == null ? "" : req.getServletPath());
        String formAction = "/test-proxy".equals(path) ? "/sharder-proxy" : "/sharder";
        ...
        for (String parameter : parameters) {
            ...
            String value = Convert.emptyToNull(req == null ? null : req.getParameter(parameter));
            if (value != null) {
    ...
    private static String buildStaticLinks() {
        StringBuilder buf = new StringBuilder();
        String requestTag = "";
        buf.append("<li");
        buf.append("><a href='./ALL.html' target='_self'>ALL</a></li>\n");
        buf.append("<li");
        buf.append("><a href='' target='_self' id='navi-selected'>SELECTED</a></li>\n");
        for (APITag apiTag : APITag.values()) {
            if (requestTags.get(apiTag.name()) != null) {
                ...
                buf.append("><a target='_self' href='./").append(apiTag.name()).append(".html'>");
    ...
    private static void buildHtml(String apiTagName, Path appRootPath) {
        PrintWriter writer = null;
        try {

            if (requestTags.get(apiTagName) != null || "ALL".equals(apiTagName)) {
                writer = new PrintWriter(appRootPath.resolve(apiTagName + ".html").toFile());
                writer.print(header1);
                writer.print(buildStaticLinks());
                ...
                bufJSCalls.append("    $('#servletPath').val('").append("');");

                if ("ALL".equals(apiTagName)) {
                    for (String type : allRequestTypes) {
                        APIServlet.APIRequestHandler requestHandler = APIServlet.apiRequestHandlers.get(type);
                        writer.print(form(null, type, false, requestHandler));
                        bufJSCalls.append("    ATS.apiCalls.push('").append(type).append("');\n");
                    }
                } else {
                    Set<String> taggedTypes = requestTags.get(apiTagName);
    ...
    protected JSONStreamAware processRequest(HttpServletRequest request) throws ConchException {
        ...
        Path appRootPath = Paths.get(rootPath);
        buildHtml("ALL", appRootPath);
        for (APITag apiTag : APITag.values()) {
            buildHtml(apiTag.name(), appRootPath);
        }
        return null;
    }
    ...
    private void createFile(String dirPath, String fileName) {
        ...
        File htmlFile = new File(dirPath + fileName);
        if (!htmlFile.exists()) {
            try {
    ...
    private void init() {
        if (isInit && allRequestTypes != null && requestTags != null) {
            return;
        }
        allRequestTypes = new ArrayList<>(APIServlet.apiRequestHandlers.keySet());
