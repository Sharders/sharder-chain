...
public final class ShufflingParticipant {
    ...
    public void setData(byte[][] data, int timestamp) {
        if (data != null && Conch.getEpochTime() - timestamp < Constants.MAX_PRUNABLE_LIFETIME && getData() == null) {
            shufflingDataTable.insert(new ShufflingData(shufflingId, accountId, data, timestamp, Conch.getBlockchain().getHeight()));
        }
    }
    ...
    public void cancel(byte[][] blameData, byte[][] keySeeds) {
    ...
    public void setProcessed(byte[] dataTransactionFullHash) {
