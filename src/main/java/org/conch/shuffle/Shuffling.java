...
import org.conch.crypto.AnonymouslyEncryptedData;
...
public final class Shuffling {
    ...
    private static final boolean deleteFinished = Conch.getBooleanProperty("sharder.deleteFinishedShufflings");
    ...
    private void cancel(Block block) {
        ...
        if (blamedAccountId != 0) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                Logger.logDebugMessage("Shuffling penalty %f %s awarded to miner at height %d", ((double)fee) / Constants.ONE_SS, Conch.COIN_UNIT, block.getHeight() - i - 1);
            }
            ...
            Logger.logDebugMessage("Shuffling penalty %f %s awarded to miner at height %d", ((double)fee) / Constants.ONE_SS, Conch.COIN_UNIT, block.getHeight());
        }
